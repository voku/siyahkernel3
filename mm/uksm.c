					 ...
					 int unlink_rb,  int remove_tree_node)
{
	struct node_vma *node_vma;
	struct rmap_item *rmap_item;
	struct hlist_node *n, *hlist;

	if (!hlist_empty(&stable_node->hlist)) {
		hlist_for_each_entry_safe(node_vma, hlist,
					  n, &stable_node->hlist, hlist) {
			hlist_for_each_entry(rmap_item, hlist, &node_vma->rmap_hlist, hlist) {
				uksm_pages_sharing--;
			       ...
			       struct stable_node *stable_node, int logdedup)
{
	...
	if (node_vma) {
		if (node_vma->key == key) {
			node_vma_cont = hlist_entry(node_vma->hlist.next, struct node_vma, hlist);
			goto node_vma_ok;
...
node_vma_ok: /* ok, ready to add to the list */
	...
	if (logdedup) {
		...
		if (node_vma_cont) {
			...
			hlist_for_each_entry_continue(node_vma, hlist, hlist) {
				node_vma->slot->pages_bemerged += factor;
				if (list_empty(&node_vma->slot->dedup_list))
					list_add(&node_vma->slot->dedup_list,
						 &vma_slot_dedup);

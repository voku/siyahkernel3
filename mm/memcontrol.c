...
mem_cgroup_get_zonestat_node(struct mem_cgroup *memcg, int nid, enum lru_list idx)
{
	struct mem_cgroup_per_zone *mz;
	u64 total = 0;
	int zid;
...
static unsigned long mem_cgroup_get_local_zonestat(struct mem_cgroup *memcg,
					enum lru_list idx)
{
	int nid;
	u64 total = 0;
	...
	for_each_online_node(nid)
		total += mem_cgroup_get_zonestat_node(memcg, nid, idx);
	return total;
}
...
/* The caller has to guarantee "memcg" exists before calling this */
...
static bool mem_cgroup_under_move(struct mem_cgroup *memcg)
{
	...
	if (!from)
		goto unlock;
	if (from == memcg || to == memcg
	    || (memcg->use_hierarchy && css_is_ancestor(&from->css, &memcg->css))
	    || (memcg->use_hierarchy && css_is_ancestor(&to->css,	&memcg->css)))
		ret = true;
unlock:
	spin_unlock(&mc.lock);
	return ret;
...
static bool test_mem_cgroup_node_reclaimable(struct mem_cgroup *memcg,
		int nid, bool noswap)
{
	if (mem_cgroup_node_nr_file_lru_pages(memcg, nid))
		return true;
	if (noswap || !total_swap_pages)
		return false;
	if (mem_cgroup_node_nr_anon_lru_pages(memcg, nid))
		return true;
	return false;
	...
	unsigned mode, int sync, void *arg)
{
	...
	if (oom_wait_info->memcg == wake_mem)
		goto wakeup;
	/* if no hierarchy, no match */
	if (!oom_wait_info->memcg->use_hierarchy || !wake_mem->use_hierarchy)
		return 0;
	/*
	 * Both of oom_wait_info->memcg and wake_mem are stable under us.
	 * Then we can use css_is_ancestor without taking care of RCU.
	 */
	if (!css_is_ancestor(&oom_wait_info->memcg->css, &wake_mem->css) &&
	    !css_is_ancestor(&wake_mem->css, &oom_wait_info->memcg->css))
		return 0;
...
{
	if (memcg && atomic_read(&memcg->oom_lock))
...
static void drain_all_stock_async(struct mem_cgroup *root_mem)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (memcg != root_mem) {
			if (!root_mem->use_hierarchy)
				continue;
			/* check whether "memcg" is under tree of "root_mem" */
			if (!css_is_ancestor(&memcg->css, &root_mem->css))
				continue;
		}
					    ...
					    unsigned long *total_scanned)
{
	...
	do {
		...
		nr_scanned = 0;
		reclaimed = mem_cgroup_hierarchical_reclaim(mz->memcg, zone,
						gfp_mask,
...
move_account:
	do {
		...
		memcg_oom_recover(memcg);
		/* it seems parent cgroup doesn't have enough memcg */
		if (ret == -ENOMEM)
			goto try_to_free;
		cond_resched();
...
mem_cgroup_get_local_stat(struct mem_cgroup *memcg, struct mcs_total_stat *s)
{
	...
	/* per zone stat */
	val = mem_cgroup_get_local_zonestat(memcg, LRU_INACTIVE_ANON);
	s->stat[MCS_INACTIVE_ANON] += val * PAGE_SIZE;
	val = mem_cgroup_get_local_zonestat(memcg, LRU_ACTIVE_ANON);
	s->stat[MCS_ACTIVE_ANON] += val * PAGE_SIZE;
	val = mem_cgroup_get_local_zonestat(memcg, LRU_INACTIVE_FILE);
	s->stat[MCS_INACTIVE_FILE] += val * PAGE_SIZE;
	val = mem_cgroup_get_local_zonestat(memcg, LRU_ACTIVE_FILE);
	s->stat[MCS_ACTIVE_FILE] += val * PAGE_SIZE;
	val = mem_cgroup_get_local_zonestat(memcg, LRU_UNEVICTABLE);

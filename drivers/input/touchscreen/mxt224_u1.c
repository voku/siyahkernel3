...
uint8_t calibrate_chip(void)
{
	...
	if (cal_check_flag == 0) {
		...
		if (copy_data->family_id == 0x81) {	/* mxT224E */
			/* forced cal thr  */
			ret = write_mem(copy_data, obj_address + 8, 1, &atchcalst_tmp);
			/* forced cal thr ratio */
			ret1 = write_mem(copy_data, obj_address + 9, 1, &atchcalsthr_tmp);
		}
	}
...
static void mxt224_ta_probe(bool ta_status)
{
	...
	if (copy_data->family_id == 0x81) {
		...
		if (copy_data->freq_table.fherr_setting >= 1) {
			...
			write_mem(copy_data, obj_address + 11, 5, &copy_data->freq_table.freq_for_fherr1[0]);
		...
		if ((tch_ch + atch_ch) > 21) {
			...
			if (qt_timer_state == 1) {
				...
				if (qt_time_diff > 300) {
					...
					if (copy_data->family_id == 0x81) {
						write_mem(copy_data, object_address + 6, 1, &copy_data->atchcalst_e);
						write_mem(copy_data, object_address + 7, 1, &copy_data->atchcalsthr_e);
...
static void report_input_data(struct mxt224_data *data)
{
	...
	)
	{
		...
		if (qt_time_diff_freq > 5000) {
			get_object_info(copy_data, GEN_ACQUISITIONCONFIG_T8, &size, &object_address);
			value = 0;
			/* TCHAUTOCAL disable */
			write_mem(copy_data, object_address + 4, 1, &value);
			copy_data->freq_table.fherr_setting = 1;
			printk(KERN_ERR"[TSP] auto cal disable\n");
			get_object_info(copy_data, PROCG_NOISESUPPRESSION_T22, &size, &object_address);
			value = 60;
			write_mem(copy_data, object_address + 8, 1, &value);
		}
		if (copy_data->freq_table.fherr_setting == 2) {
			write_mem(copy_data, copy_data->cmd_proc + CMD_CALIBRATE_OFFSET, 1, &copy_data->freq_table.fherr_num);
	...
	if (track_gestures) {
		...
		for (gesture_no = 0; gesture_no <= max_configured_gesture; gesture_no++) {
			...
			for (finger_no = 0; finger_no <= max_gesture_finger[gesture_no]; finger_no++) {
				if (
					gestures_step_count[gesture_no][finger_no] > 0 &&
				    gesture_fingers[gesture_no][finger_no].current_step < gestures_step_count[gesture_no][finger_no]
				) 
				{
...
void palm_recovery(void)
{
	...
	if (copy_data->palm_chk_flag == 2) {
		...
		if (cal_check_flag == 0) {
			...
			/* TCHAUTOCAL */
			ret = write_mem(copy_data, obj_address + 4, 1, &atchcalst_tmp);	
...
static int freq_hop_err_setting(int state)
{
	...
	ret = get_object_info(copy_data, GEN_POWERCONFIG_T7, &size_one, &object_address);
	value = 255;
	...
	ret = get_object_info(copy_data, GEN_ACQUISITIONCONFIG_T8, &size_one, &object_address);
	value = 5;
	/* TCHAUTOCAL 1sec */
	write_mem(copy_data, object_address + 4, 1, &value);
	...
	ret = get_object_info(copy_data, PROCG_NOISESUPPRESSION_T22, &size_one, &object_address);
	value = 135;
	...
	if (state == 1) {
		write_mem(copy_data, object_address + 11, 5, &copy_data->freq_table.freq_for_fherr1[0]);
	} else if (state == 2) {
		write_mem(copy_data, object_address + 11, 5, &copy_data->freq_table.freq_for_fherr2[0]);
		copy_data->freq_table.fherr_num = 1;
		copy_data->freq_table.fherr_cnt = 2;
	} else if (state == 3) {
		write_mem(copy_data, object_address + 11, 5, &copy_data->freq_table.freq_for_fherr3[0]);
		copy_data->freq_table.fherr_num = 1;
	} else if (state == 4) {
		write_mem(copy_data, object_address + 11, 5, &copy_data->freq_table.freq_for_fherr4[0]);
		copy_data->freq_table.fherr_num = 1;
		copy_data->freq_table.fherr_cnt = 0;
	}
...
static void median_err_setting(void)
{
	...
	if (cal_check_flag == 1) {
		...
		ret = get_object_info(copy_data, GEN_ACQUISITIONCONFIG_T8, &size_one, &obj_address);
		write_mem(copy_data, obj_address + 6, 1, &copy_data->atchcalst_e);
		write_mem(copy_data, obj_address + 7, 1, &copy_data->atchcalsthr_e);
		write_mem(copy_data, obj_address + 8, 1, &copy_data->atchfrccalthr_e);
		write_mem(copy_data, obj_address + 9, 1, &copy_data->atchfrccalratio_e);
	}
	...
	if (!ta_status_check) {
		...
		)
		{
			...
			ret |= get_object_info(copy_data, PROCG_NOISESUPPRESSION_T48, &size_one, &obj_address);
			if (state == 1) {
				value = copy_data->noise_median.t48_mfinvlddiffthr_for_mferr;
				write_mem(copy_data, obj_address + 22, 1, &value);
				value = copy_data->noise_median.t48_mferrorthr_for_mferr;
				write_mem(copy_data, obj_address + 25, 1, &value);
				value = copy_data->noise_median.t48_thr_for_mferr;
				write_mem(copy_data, obj_address + 35, 1, &value);
				value = copy_data->noise_median.t48_movfilter_for_mferr;
				write_mem(copy_data, obj_address + 39, 1, &value);
				ret |= get_object_info(copy_data, SPT_CTECONFIG_T46, &size_one, &obj_address);
				value = copy_data->noise_median.t46_actvsyncsperx_for_mferr;
				write_mem(copy_data, obj_address + 3, 1, &value);
			} else if (state >= 2) {
				value = 10;
				write_mem(copy_data, obj_address + 3, 1, &value);
...
static irqreturn_t mxt224_irq_thread(int irq, void *ptr)
{
	...
	u16 size_one;
	u16 obj_address = 0;
	int ta_status_check, freq_hop_err;
	...
	do {
		...
		if ((msg[0] == 0xf) && (copy_data->family_id == 0x80)) {
			if ((msg[1]&0x08) == 0x08) {
				...
				if (copy_data->freq_table.fherr_cnt > (copy_data->freq_table.fherr_num * 4))
					copy_data->freq_table.fherr_cnt = 1;

				if (!(copy_data->freq_table.fherr_cnt % copy_data->freq_table.fherr_num)) {
					freq_hop_err = freq_hop_err_setting(copy_data->freq_table.fherr_cnt / copy_data->freq_table.fherr_num);
		...
		if ((msg[0] == 18) && (data->family_id == 0x81)) {
			if ((msg[4] & 0x5) == 0x5) {
				...
				)
				{
					...
					value = 0;
					write_mem(copy_data, obj_address + 34, 1, &value);
					copy_data->noise_median.median_on_flag = true;
				}
			}
		}
#endif
		...
		if (msg[0] > 1 && msg[0] < 12) {
			...
			if (
				(copy_data->touch_is_pressed_arr[msg[0] - 2] == 1) && 
				(msg[1] & 0x40)
			)
			{
			...
			*/
			...
			if (msg[1] & RELEASE_MSG_MASK) {
				...
				(msg[1] & DETECT_MSG_MASK) && 
				(msg[1] & (PRESS_MSG_MASK | MOVE_MSG_MASK))
			)
			{
				if (msg[1] & PRESS_MSG_MASK) {
					copy_data->touch_is_pressed_arr[id] = TSP_STATE_PRESS;
				} else if (msg[1] & MOVE_MSG_MASK) {
					copy_data->touch_is_pressed_arr[id] = TSP_STATE_MOVE;
				}
			...
			)
			{
				...
				data->fingers[id].w = msg[5];
				data->finger_mask |= 1U << id;
				copy_data->touch_is_pressed_arr[id] = TSP_STATE_RELEASE;
			} else {
...
void read_dbg_data(uint8_t dbg_mode, uint8_t node, uint16_t *dbg_data)
{
	...
	if (!copy_data->mxt224_enabled) {
		printk(KERN_ERR "[TSP ]read_dbg_data. copy_data->mxt224_enabled is 0\n");
		return;
	}
...
/* jerry no need to leave it */
	...
	for (read_page = 0; read_page < 4; read_page++) {
		for (node = 0; node < 64; node++) {
			...
			if (copy_data->family_id == 0x81) {
				if (
					(qt_refrence_node[num] > MIN_VALUE + 16384) || 
					(qt_refrence_node[num] < MAX_VALUE + 16384)
				)
				{
					state = 1;
				...
				if (
					(qt_refrence_node[num] > MIN_VALUE) || 
...
static int mxt224_load_fw(struct device *dev, const char *fn)
{
	...
	while (pos < fw->size) {
		ret = mxt224_check_bootloader(client, QT602240_WAITING_FRAME_DATA);
		if (ret)
			goto out;
				      ...
				      const char *buf, size_t size)
{
	...
	if (!copy_data->mxt224_enabled) {
		printk(KERN_ERR "[TSP ]set_tsp_for_inputmethod_store. copy_data->mxt224_enabled is 0\n");
		return 1;
	}
...
static int sec_touchscreen_open(struct input_dev *dev)
{
	...
	if (ret > 0) {
		...
		dev_err(&dev->dev, "error while waiting for device to init (%d)\n", ret);
		ret = -ENXIO;
	} else if (ret == 0) {
		dev_err(&dev->dev, "timedout while waiting for device to init\n");
		ret = -ENXIO;
	}
                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	ret = wait_event_interruptible(gestures_wq, has_gestures);
	if (ret)
...
*/
	...
	if (IS_ERR(tsp_noise_test))
		printk(KERN_ERR "Failed to create device(tsp_noise_test)!\n");
	...
	if (device_create_file(tsp_noise_test, &dev_attr_set_all_refer) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
		       dev_attr_set_all_refer.attr.name);

	if (device_create_file(tsp_noise_test, &dev_attr_disp_all_refdata) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
		       dev_attr_disp_all_refdata.attr.name);

	if (device_create_file(tsp_noise_test, &dev_attr_set_all_delta) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
	...
	if (device_create_file(tsp_noise_test, &dev_attr_set_threshould) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
		       dev_attr_set_threshould.attr.name);

	if (device_create_file(tsp_noise_test, &dev_attr_set_firm_version) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
		       dev_attr_set_firm_version.attr.name);

	if (device_create_file(tsp_noise_test, &dev_attr_set_module_off) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
		       dev_attr_set_threshould.attr.name);

	if (device_create_file(tsp_noise_test, &dev_attr_set_module_on) < 0)
		printk(KERN_ERR "Failed to create device file(%s)!\n",
		       dev_attr_set_firm_version.attr.name);
#endif

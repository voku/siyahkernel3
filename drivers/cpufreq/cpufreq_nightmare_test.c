...
static void nightmare_check_cpu(struct cpufreq_nightmare_cpuinfo *this_nightmare_cpuinfo)
{
	...
	int cpu = this_nightmare_cpuinfo->cpu;
	...
	hotplug_history->usage[num_hist].freq[cpu] = 0;
	hotplug_history->usage[num_hist].load[cpu] = 0;
	...
	if ((int)nightmare_tuners_ins.ignore_nice) {
		...
		cur_nice = kcpustat_cpu(cpu).cpustat[CPUTIME_NICE] -
					 this_nightmare_cpuinfo->prev_cpu_nice;
		/*
		...
		this_nightmare_cpuinfo->prev_cpu_nice = kcpustat_cpu(cpu).cpustat[CPUTIME_NICE];
	...
	hotplug_history->usage[num_hist].freq[cpu] = this_nightmare_cpuinfo->cur_policy->cur;
	hotplug_history->usage[num_hist].load[cpu] = cur_load;

	/* calculate the scaled load across CPU */
	load_at_max_freq = (cur_load * this_nightmare_cpuinfo->cur_policy->cur)/
				this_nightmare_cpuinfo->cur_policy->cpuinfo.max_freq;
	cpufreq_notify_utilization(this_nightmare_cpuinfo->cur_policy, load_at_max_freq);

	if (hotplug_enable && cpu == 0) {
		/* Check for CPU hotplug */
		if (check_up()) {
			queue_work_on(cpu, dvfs_workqueue,
...
static void nightmare_check_frequency(struct cpufreq_nightmare_cpuinfo *this_nightmare_cpuinfo)
{
	...
	struct cpufreq_policy *policy = this_nightmare_cpuinfo->cur_policy;
	u8 cur_load = 0;
	u8 inc_cpu_load = 0;
	...
	cur_load = hotplug_history->usage[num_hist].load[cpu];
...
static inline void nightmare_timer_init(struct cpufreq_nightmare_cpuinfo *nightmare_cpuinfo)
{
	...
	for_each_possible_cpu(cpu) {
		...
		j_nightmare_cpuinfo = &per_cpu(od_nightmare_cpuinfo, cpu);
		j_nightmare_cpuinfo->cpu = cpu;
		if (cpu_online(cpu)) {
			INIT_DEFERRABLE_WORK(&j_nightmare_cpuinfo->work, do_nightmare_timer);
			if (cpu == 0) {
				INIT_WORK(&j_nightmare_cpuinfo->up_work, cpu_up_work);
				INIT_WORK(&j_nightmare_cpuinfo->down_work, cpu_down_work);
			}
			queue_delayed_work_on(j_nightmare_cpuinfo->cpu, dvfs_workqueue, &nightmare_cpuinfo->work, delay);
		}
	}
}

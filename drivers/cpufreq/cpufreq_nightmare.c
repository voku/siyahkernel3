...
static void nightmare_check_cpu(struct cpufreq_nightmare_cpuinfo *this_nightmare_cpuinfo)
{
	...
	for_each_online_cpu(j) {
		...
		hotplug_history->usage[num_hist].freq[j] = j_nightmare_cpuinfo->cpu_policy->cur;
		hotplug_history->usage[num_hist].load[j] = cur_load;
		// GET MIN MAX FREQ
		min_freq = j_nightmare_cpuinfo->cpu_policy->min;
		max_freq = j_nightmare_cpuinfo->cpu_policy->max;
		if (earlysuspend) {
			min_freq = min(j_nightmare_cpuinfo->cpu_policy->min_suspend,min_freq);
			max_freq = min(j_nightmare_cpuinfo->cpu_policy->max_suspend,max_freq);
		}
		/* CPUs Online Scale Frequency*/
		if (j_nightmare_cpuinfo->cpu_policy->cur < freq_for_responsiveness) {
		...
		} else if (j_nightmare_cpuinfo->cpu_policy->cur > freq_for_responsiveness_max) {
		...
		if (cur_load >= inc_cpu_load) {
			...
			if (j_nightmare_cpuinfo->cpu_policy->cur == max_freq) {
				continue;
			}

			next_freq = nightmare_frequency_adjust(j_nightmare_cpuinfo->cpu_policy->cur + (((cur_load + freq_step - freq_up_brake) * 200000) / 100), j_nightmare_cpuinfo->cpu_policy->cur, min_freq, max_freq, up_sf_step);
			/* printk(KERN_ERR "UP FREQ CALC.: CPU[%u], load[%d]>=inc_cpu_load[%d], target freq[%u], cur freq[%u], min freq[%u], max_freq[%u]\n",j, cur_load, inc_cpu_load, freq_up, j_nightmare_cpuinfo->cpu_policy->cur, min_freq, max_freq); */
			if (next_freq != j_nightmare_cpuinfo->cpu_policy->cur && next_freq != prev_freq_set) {				
				__cpufreq_driver_target(j_nightmare_cpuinfo->cpu_policy, next_freq, CPUFREQ_RELATION_L);
			}
		} else if (cur_load < dec_cpu_load) {
			/* if we cannot reduce the frequency anymore, break out early */
			if (j_nightmare_cpuinfo->cpu_policy->cur == min_freq) {
				continue;
			}

			next_freq = nightmare_frequency_adjust(j_nightmare_cpuinfo->cpu_policy->cur - (((100 - cur_load + freq_step_dec) * 200000) / 100), j_nightmare_cpuinfo->cpu_policy->cur, min_freq, max_freq, down_sf_step);
			/* printk(KERN_ERR "DOWN FREQ CALC.: CPU[%u], load[%d]<dec_cpu_load[%d], target freq[%u], cur freq[%u], min freq[%u], max_freq[%u]\n",j, cur_load, dec_cpu_load, freq_down, j_nightmare_cpuinfo->cpu_policy->cur, min_freq, max_freq); */
			if (next_freq < j_nightmare_cpuinfo->cpu_policy->cur && next_freq != prev_freq_set) {
				__cpufreq_driver_target(j_nightmare_cpuinfo->cpu_policy, next_freq, CPUFREQ_RELATION_L);				
			}
		}
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_possible_cpu(j) {
			struct cpufreq_nightmare_cpuinfo *j_nightmare_cpuinfo;
			j_nightmare_cpuinfo = &per_cpu(od_nightmare_cpuinfo, j);
			j_nightmare_cpuinfo->cpu_policy = policy;
	...
	case CPUFREQ_GOV_LIMITS:
	    ...
	    if (max_freq < this_nightmare_cpuinfo->cpu_policy->cur) {
	      __cpufreq_driver_target(this_nightmare_cpuinfo->cpu_policy,max_freq,CPUFREQ_RELATION_H);
	    } else if (min_freq > this_nightmare_cpuinfo->cpu_policy->cur) {
	      __cpufreq_driver_target(this_nightmare_cpuinfo->cpu_policy,min_freq,CPUFREQ_RELATION_L);
	    }
